...
def individual_prediction_tab(model: Pipeline) -> None:
    ...
    if prediction == zero:
        st.error(
            "⚠️ The model predicted a price of $0. "
            + "This result is invalid or unreliable. "
        ...
        try:
            ...
            if missing_cols:
                ...
                if any(p == 0 for p in cleaned_predictions):
                    st.warning(
                        "⚠️ One or more predictions resulted in $0. These values "
                        "may be invalid. Please check the input data for potential "
                        "inconsistencies."
                    )
...
def main() -> None:
    ...
    st.write(
        "Estimate the sale price of a property in New York City using our "
